from app.calculations import add_two_numbers, subtract, multiply, divide, BankAccount, InsufficientFunds
import pytest


@pytest.fixture
def zero_bank_account():
    return BankAccount()


@pytest.fixture
def bank_account():
    return BankAccount(1000)


@pytest.mark.parametrize("num1, num2, expected", [
    (3, 2, 5), (7, 1, 8), (12, 4, 16)
])
def test_add(num1, num2, expected):
    assert add_two_numbers(num1, num2) == expected


def test_subtract():
    assert subtract(1, 2) == -1


def test_divide():
    assert divide(1, 2) == 0.5


def test_multiply():
    assert multiply(1, 2) == 2


def test_bank_set_initial_amount(bank_account):
    assert bank_account.balance == 1000


def test_bank_default_amount(zero_bank_account):
    assert zero_bank_account.balance == 0


def test_withdraw():
    bank_account = BankAccount(1000)
    bank_account.withdraw(100)
    assert bank_account.balance == 900


def test_deposit():
    bank_account = BankAccount(1000)
    bank_account.deposit(100)
    assert bank_account.balance == 1100


def test_collect_interest():
    bank_account = BankAccount(1000)
    bank_account.collect_interest()
    assert round(bank_account.balance, 5) == 1100


@pytest.mark.parametrize("deposited, withdrew, expected", [
    (200, 100, 100), (50, 10, 40), (1200, 200, 1000)
])
def test_bank_transaction(zero_bank_account, deposited, withdrew, expected):
    zero_bank_account.deposit(deposited)
    zero_bank_account.withdraw(withdrew)
    assert zero_bank_account.balance == expected


def test_insufficient_funds(bank_account):
    with pytest.raises(InsufficientFunds):
        bank_account.withdraw(1200)
